function def(cmd, user, users, bot, channelID, evt){
	...
	https.get('https://en.wikipedia.org/w/api.php?action=opensearch&search='+cmd.substring(8)+'&limit=1&namespace=0&format=json', (resp) => {
					let data = '';
					let exdata = '';
					let extract = '';
						...
						if(!jsondata[2].toString().length){
							if(!jsondata[3].toString().length){
								...
								https.get('https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles='+jsondata[3].toString().substring(30), (aditional) => {
									...
									aditional.on("end", () =>{
										exjsondata = JSON.parse(exdata);
						
										for (let k in exjsondata.query.pages){
